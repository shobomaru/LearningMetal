...
class MyResource {
    ...
    var psoPreTess: MTLComputePipelineState?
    var vb: MTLBuffer
    var ib: MTLBuffer
    ...
    init(device: MTLDevice, alert: (String) -> Void) {
        ...
        guard let csPreTess = lib.makeFunction(name: "preTessCS") else { fatalError() }
        let psoDesc = MTLRenderPipelineDescriptor()
        psoDesc.vertexFunction = vs
        psoDesc.fragmentFunction = fs
        psoDesc.maxTessellationFactor = 14
        psoDesc.tessellationPartitionMode = .fractionalEven
        psoDesc.tessellationFactorFormat = .half
        psoDesc.tessellationFactorStepFunction = .perPatchAndPerInstance
        psoDesc.tessellationOutputWindingOrder = .clockwise
        psoDesc.tessellationControlPointIndexType = .uint16
        psoDesc.isTessellationFactorScaleEnabled = false // what is this?
        ...
        do {
            ...
            self.psoPreTess = try device.makeComputePipelineState(function: csPreTess)
        } catch let e {
            print(e)
        ...
        instanceMat = instanceMat.map {_ in
            let tx = Float.random(in: -2.5...2.5, using: &rng)
            let ty = Float.random(in: -2.5...2.5, using: &rng)
            let tz = Float.random(in: -2.5...2.5, using: &rng)
            let sx = Float.random(in: 0.15...0.25, using: &rng)
            let sy = Float.random(in: 0.15...0.25, using: &rng)
            let sz = Float.random(in: 0.15...0.25, using: &rng)
        ...
        self.tessFactorBuf = device.makeBuffer(length: MemoryLayout<MTLTriangleTessellationFactorsHalf>.stride * InstanceCount * (6 * SPHERE_SLICES * SPHERE_STACKS / 3), options: .storageModePrivate)!
    }
    func available() -> Bool {
        self.pso != nil && self.psoPreTess != nil
    }
}

let SPHERE_STACKS: Int = 4
let SPHERE_SLICES: Int = 5
...
class Metal: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        csEnc.label = "PreTess Pass"
        csEnc.setComputePipelineState(self.resource.psoPreTess!)
        csEnc.setBuffer(self.resource.vb, offset: 0, index: 0)
        csEnc.setBuffer(self.resource.ib, offset: 0, index: 1)
        csEnc.setBuffer(self.resource.instanceMat, offset: 0, index: 2)
        csEnc.setBuffer(self.resource.tessFactorBuf, offset: 0, index: 3)
        struct CInfo {
            var aspectRatio: Float
            var primitiveCount: UInt16
            var padding: UInt16 = 0
        }
        var infoData = CInfo(aspectRatio: Float(view.drawableSize.width / view.drawableSize.height), primitiveCount: UInt16(6 * SPHERE_SLICES * SPHERE_STACKS / 3))
        csEnc.setBytes(&infoData, length: MemoryLayout.size(ofValue: infoData), index: 4)
        csEnc.setBuffer(self.resource.cbScene[frameIndex], offset: 0, index: 5)
        csEnc.dispatchThreadgroups(MTLSize(width: (Int(infoData.primitiveCount) + 32 - 1) / 32, height: InstanceCount, depth: 1), threadsPerThreadgroup: MTLSize(width: 32, height: 1, depth: 1))
        csEnc.endEncoding()
        ...
        //enc.setTriangleFillMode(.lines) // for visualizing
        ...
        enc.setTessellationFactorBuffer(self.resource.tessFactorBuf, offset: 0, instanceStride: MemoryLayout<MTLTriangleTessellationFactorsHalf>.size * 6 * SPHERE_SLICES * SPHERE_STACKS / 3)
        // If you want to use a patchIndexBuffer, buffer's element must be 32bit
        // One the other hand, controlPointIndexBuffer can use 16bit which can be selected by PSO
        enc.drawIndexedPatches(numberOfPatchControlPoints: 3, patchStart: 0, patchCount: 6 * SPHERE_SLICES * SPHERE_STACKS / 3, patchIndexBuffer: nil, patchIndexBufferOffset: 0, controlPointIndexBuffer: self.resource.ib, controlPointIndexBufferOffset: 0, instanceCount: InstanceCount, baseInstance: 0)
        enc.endEncoding()
