...
class MyResource {
    ...
    init(device: MTLDevice, alert: (String) -> Void) {
        ...
        guard let os = lib.makeFunction(name: "sceneOS") else { fatalError() } // a.k.a. Amplification Shader or Task Shader
        ...
        psoDesc.objectThreadgroupSizeIsMultipleOfThreadExecutionWidth = true
        ...
        do {
            ...
            if 32 % self.psoMS!.meshThreadExecutionWidth != 0 {
                fatalError()
            }
        } catch let e {
            print(e)
        ...
        let objectInfoData = [UInt32(self.meshletCount), UInt32(InstanceCount)]
        self.objectInfo = device.makeBuffer(bytes: objectInfoData, length: (MemoryLayout<UInt32>.size * objectInfoData.count), options: [.cpuCacheModeWriteCombined])!
...
class Metal: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        enc.setObjectBuffer(self.resource.meshlet, offset: 0, index: 0)
        enc.setObjectBuffer(self.resource.instanceMat, offset: 0, index: 2)
        enc.setObjectBuffer(self.resource.objectInfo, offset: 0, index: 3)
        ...
        let dispatchY = (InstanceCount + 1) / 2 // Object shader emits up to 2 instances
        let dispatchX = self.resource.meshletCount
        enc.drawMeshThreadgroups(MTLSizeMake(dispatchX, dispatchY, 1), threadsPerObjectThreadgroup: MTLSizeMake(32, 1, 1), threadsPerMeshThreadgroup: self.resource.numThreads)
        enc.endEncoding()
