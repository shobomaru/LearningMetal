...
class MyResource {
    ...
    var psoResolve: MTLRenderPipelineState?
    var vb: MTLBuffer
    var ib: MTLBuffer
    ...
    init(device: MTLDevice, alert: (String) -> Void) {
        ...
        do {
            self.psoResolve = try device.makeRenderPipelineState(descriptor: psoDesc)
        } catch let e {
            print(e)
            alert(String(describing: e))
        }
...
class Metal: NSObject, MTKViewDelegate {
    ...
    func mtkView(_ view: MTKView, drawableSizeWillChange size: CGSize) {
        ...
        texDesc.width = (Int(size.width) + 1) / 2
        texDesc.height = (Int(size.height) + 1) / 2
        texDesc.textureType = .type2DMultisample
        texDesc.sampleCount = 2
        ...
        self.msaaTex = [MTLTexture](closure: { self.device.makeTexture(descriptor: texDesc)! }, count: 2)
    }
    func draw(in view: MTKView) {
        ...
        // Inspired from Rainbow Six Siege
        // https://ubm-twvideo01.s3.amazonaws.com/o1/vault/gdc2016/Presentations/El_Mansouri_Jalal_Rendering_Rainbow_Six.pdf
        
        var samplePos = [MTLSamplePosition(x: 0.25, y: 0.25), MTLSamplePosition(x: 0.75, y: 0.75)]
        if frameIndex == 1 {
            samplePos[0] = MTLSamplePosition(x: 0.75, y: 0.25)
            samplePos[1] = MTLSamplePosition(x: 0.25, y: 0.75)
        }
        var passDesc = MTLRenderPassDescriptor()
        passDesc.colorAttachments[0].texture = self.msaaTex[frameIndex]
        ...
        passDesc.setSamplePositions(samplePos)
        ...
        var isOdd = [UInt32(frameIndex)]
        enc.setFragmentBytes(&isOdd, length: 4, index: 1)
        ...
        // Resolve
        enc.setRenderPipelineState(self.resource.psoResolve!)
        enc.setFragmentTexture(self.msaaTex[1 - frameIndex], index: 0)
        enc.setFragmentTexture(self.msaaTex[frameIndex], index: 1)
        enc.setFragmentBytes(&isOdd, length: 4, index: 0)
        enc.drawPrimitives(type: .triangle, vertexStart: 0, vertexCount: 3)
        enc.endEncoding()
