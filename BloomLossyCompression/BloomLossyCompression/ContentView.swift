...
class MyResource {
    ...
    var psoStretchRgba8: MTLRenderPipelineState?
    var psoStretchBlend: MTLRenderPipelineState?
    var psoStretchBlendRgba8: MTLRenderPipelineState?
    ...
    init(device: MTLDevice, alert: (String) -> Void) {
        ...
        psoDesc.label = "StretchRGBA8 PSO"
        do {
            self.psoStretchRgba8 = try device.makeRenderPipelineState(descriptor: psoDesc)
        } catch let e {
            print(e)
            alert(String(describing: e))
        }
        ...
        psoDesc.colorAttachments[0].pixelFormat = .rgba8Unorm
        psoDesc.label = "StretchBlend RGBA8 PSO"
        do {
            self.psoStretchBlendRgba8 = try device.makeRenderPipelineState(descriptor: psoDesc)
        } catch let e {
            print(e)
            alert(String(describing: e))
        }
        ...
        let calcDownsampleHeapSize = { () -> Int in
            ...
            for i in 0..<DownsampleLevel {
                ...
                desc.width = BloomTexSize / (i + 1)
                desc.height = BloomTexSize / (i + 1)
    ...
    func available() -> Bool {
        self.psoStretch != nil && self.psoStretchRgba8 != nil && self.psoStretchBlend != nil && self.psoStretchBlendRgba8 != nil && self.psoBlur != nil
    }
}
...
class Metal: NSObject, MTKViewDelegate {
    ...
    init(_ parent: ContentView2) {
        ...
        #endif
        if ManualTracking && !device.supportsFamily(.apple8) {
            parent.enqueueAlert("Lossy compression not supported (Apple A15 or M2+)")
        }
    ...
    func draw(in view: MTKView) {
        ...
        // Draw a base texture
        enc.setRenderPipelineState(self.resource.psoStretchRgba8!)
        ...
        enc.setRenderPipelineState(self.resource.psoStretchBlendRgba8!)
        enc.useHeap(self.resource.heap, stages: .fragment)
        enc.setFragmentBytes([Float](repeating: 0.0, count: 1), length: 4, index: 0)
