...
func normalize(_ v: simd_float3) -> simd_float3 {
    let t = v.x * v.x + v.y * v.y + v.z * v.z
    let r = sqrt(t)
    return simd_float3(v.x / r, v.y / r, v.z / r)
}
#endif
...
class MyResource {
    ...
    init(device: MTLDevice, alert: (String) -> Void) {
        ...
        sailboatTexDesc.usage = Int(MTLTextureUsage.shaderRead.rawValue)
        ...
        lennaTexDesc.usage = Int(MTLTextureUsage.shaderRead.rawValue)
...
class Metal: NSObject, MTKViewDelegate {
    ...
    func mtkView(_ view: MTKView, drawableSizeWillChange size: CGSize) {
        ...
        texDesc.usage = Int(MTLTextureUsage.renderTarget.rawValue)
