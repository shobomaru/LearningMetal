...
struct Output {
    ...
    ushort zid [[render_target_array_index]];
    float2 texcoord;
    float texcoordZ;
};
...
vertex Output lut3DVS(ushort vid [[vertex_id]], ushort instanceID [[instance_id]],
                    constant uint& numSlice)
{
    float x = (vid & 2) ? 3.0 : -1.0;
    float y = (vid & 1) ? 3.0 : -1.0;
    Output output;
    output.position = float4(x, y, 0, 1);
    output.zid = instanceID;
    output.texcoord = (output.position.xy) * float2(1, -1) * 0.5 + 0.5;
    output.texcoordZ = ((float)instanceID + 0.5) / (float)numSlice;
    return output;
}
...
static float2 getTransmittanceTexUVFromRMu(float len, float viewCosAngle)
{
    ...
    float x_r = rho / h;
    return float2(x_mu, 1 - x_r); // inversed-Y
}
...
{
    float scattering_texture_mu_s_size = 1.0; // Pre-multiplied in fragCoord.x
    float frag_coord_nu = floor(fragCoord.x / scattering_texture_mu_s_size) / SCATTERING_TEXTURE_NU_SIZE;
...
fragment half4 transmittanceLutFS(Output input [[stage_in]])
{
...
fragment ColorOutput singleScatteringTexFS(Output input [[stage_in]],
                                           texture2d<half> transmittanceLut [[texture(0)]])
{
    float3 uvw = float3(input.texcoord, input.texcoordZ);
    uvw.x *= SCATTERING_TEXTURE_NU_SIZE;
    bool ray_r_mu_intersects_ground;
    float4 rMuMuSNu = getRMuMuSNuFromScatteringTextureFragCoord(uvw, ray_r_mu_intersects_ground);
    auto col = computeSingleScattering(rMuMuSNu.x, rMuMuSNu.y, rMuMuSNu.z, rMuMuSNu.w, ray_r_mu_intersects_ground, transmittanceLut);
    return {
        .rt0 = half4(col[0], 1),
        .rt1 = half4(col[1], 1),
    };
