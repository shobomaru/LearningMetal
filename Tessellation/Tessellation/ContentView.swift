...
class MyResource {
    ...
    init(device: MTLDevice, alert: (String) -> Void) {
        ...
        psoDesc.tessellationPartitionMode = .integer
        psoDesc.tessellationFactorFormat = .half
        psoDesc.tessellationFactorStepFunction = .perPatch
        ...
        let tessFactorData = [Float16](repeating: Float16(Float(4.0)), count: MemoryLayout<MTLTriangleTessellationFactorsHalf>.stride * InstanceCount)
        self.tessFactorBuf = device.makeBuffer(bytes: tessFactorData, length: MemoryLayout<MTLTriangleTessellationFactorsHalf>.stride * InstanceCount)!
    }
    func available() -> Bool {
        self.pso != nil
    }
...
class Metal: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        // for MTLTessellationFactorStepFunctionConstant and MTLTessellationFactorStepFunctionPerPatch tessellationFactorBufferInstanceStride(N) must be zero.
        enc.setTessellationFactorBuffer(self.resource.tessFactorBuf, offset: 0, instanceStride: 0)
