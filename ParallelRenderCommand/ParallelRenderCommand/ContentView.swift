...
class MyResource {
    ...
    init(device: MTLDevice, alert: (String) -> Void) {
        ...
        instanceMat = instanceMat.map {_ in
            ...
            let tz = Float.random(in: -2.5...2.5)
...
class Metal: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        let encParent = cmdBuf.makeParallelRenderCommandEncoder(descriptor: passDesc)!
        encParent.label = "Parallel Scene Pass"
        let drawSpheres = { (offset: Int, rep: Int) in
            let enc = encParent.makeRenderCommandEncoder()!
        ...
        let queues = [DispatchQueue(label: "DQ0"), DispatchQueue(label: "DQ1"), DispatchQueue(label: "DQ2"), DispatchQueue(label: "DQ3")]
        for i in 0..<queues.count {
            queues[i].async(group: group) {
                drawSpheres(i * InstanceCount / queues.count, InstanceCount / queues.count)
            }
        }
        group.wait()
        encParent.endEncoding()
