...
struct Meshlet {
    ...
    var aabbMin = MTLPackedFloat3()
    var aabbMax = MTLPackedFloat3()
    init() {
    }
    func convertGpuData() -> [UInt32] {
        ...
        let meshletDataSize = MemoryLayout<UInt32>.size
            ...
            + MemoryLayout<MTLPackedFloat3>.size
            + MemoryLayout<MTLPackedFloat3>.size
            + MemoryLayout<UInt32>.size
        ...
        data[offset + 1] = self.aabbMin.x.bitPattern
        data[offset + 2] = self.aabbMin.y.bitPattern
        data[offset + 3] = self.aabbMin.z.bitPattern
        data[offset + 4] = self.aabbMax.x.bitPattern
        data[offset + 5] = self.aabbMax.y.bitPattern
        data[offset + 6] = self.aabbMax.z.bitPattern
        data[offset + 7] = 0x10BECEE1
        offset += 8
        return data
    }
}
...
class MyResource {
    ...
    var psoLine: MTLRenderPipelineState?
    ...
    init(device: MTLDevice, alert: (String) -> Void) {
        ...
        guard let vsLine = lib.makeFunction(name: "lineVS") else { fatalError() }
        guard let fsLine = lib.makeFunction(name: "lineFS") else { fatalError() }
        ...
        let psoDesc2 = MTLRenderPipelineDescriptor()
        psoDesc2.vertexFunction = vsLine
        psoDesc2.fragmentFunction = fsLine
        psoDesc2.colorAttachments[0].pixelFormat = .rgba8Unorm
        psoDesc2.depthAttachmentPixelFormat = .depth32Float
        psoDesc2.label = "Line PSO"
        do {
            self.psoLine = try device.makeRenderPipelineState(descriptor: psoDesc2)
        } catch let e {
            print(3)
            alert(String(describing: e))
        }
    ...
    private static func generateMeshlet(vertices: [VertexElement], indices: [QuadIndexList]) -> [Meshlet] {
        ...
        while inQuadCount < indices.count {
            ...
            let aabbMin = ms.positions[0..<Int(ms.vertexCount)].reduce(MTLPackedFloat3Make(Float.infinity, Float.infinity, Float.infinity)) {
                MTLPackedFloat3Make(Float.minimum($0.x, $1.x), Float.minimum($0.y, $1.y), Float.minimum($0.z, $1.z))
            }
            let aabbMax = ms.positions[0..<Int(ms.vertexCount)].reduce(MTLPackedFloat3Make(-Float.infinity, -Float.infinity, -Float.infinity)) {
                MTLPackedFloat3Make(Float.maximum($0.x, $1.x), Float.maximum($0.y, $1.y), Float.maximum($0.z, $1.z))
            }
            ms.aabbMin = aabbMin
            ms.aabbMax = aabbMax
            //print("BBOX : \(ms.aabbMin.x), \(ms.aabbMin.y), \(ms.aabbMin.z) -> \(ms.aabbMax.x), \(ms.aabbMax.y), \(ms.aabbMax.z)")
            meshlets.append(ms)
        }
        return meshlets
    }
    func available() -> Bool {
        self.psoMS != nil && self.psoLine != nil
    }
}
...
class Metal: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        enc.setRenderPipelineState(self.resource.psoLine!)
        enc.drawPrimitives(type: .lineStrip, vertexStart: 0, vertexCount: 5)
        enc.endEncoding()
