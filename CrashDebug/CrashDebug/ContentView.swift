...
class Metal: NSObject, MTKViewDelegate {
    ...
    init(_ parent: ContentView2) {
        ...
        parent.enqueueAlert("This app will crash your GPU. OK?")
    }
    ...
    func draw(in view: MTKView) {
        // Avoid system infinite reboot running on MacOS...
        if self.parent.isShowAlert { return }
        ...
        // Enable enhanced command buffer errors
        let cmdBufDesc = MTLCommandBufferDescriptor()
        cmdBufDesc.errorOptions = .encoderExecutionStatus
        ...
        enc.pushDebugGroup("Draw Sphere") // Group cannot dump
        ...
        enc.popDebugGroup()
        enc.insertDebugSignpost("Draw Plane")
        ...
        cmdBuf.addCompletedHandler {[weak self, weak cmdBuf] _ in
            ...
            // https://developer.apple.com/videos/play/wwdc2020/10616/
            if let err = cmdBuf?.error as NSError? {
                if let encoderInfos = err.userInfo[MTLCommandBufferEncoderInfoErrorKey] as? [MTLCommandBufferEncoderInfo] {
                    print("- Start of the command buffer -")
                    for info in encoderInfos {
                        print("\(info.label) : \(info.debugSignposts.joined(separator: ","))")
                        if info.errorState == .faulted {
                            print("@@@ \(info.label) faulted!!!")
                        }
                    }
                    print("- End of the command buffer -")
                }
