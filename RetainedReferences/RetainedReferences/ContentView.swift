...
// If you set `false`, this app will crash
let ManualRetaining = true
...
class MyResource {
    ...
    var stagedRes: [[MTLResource]]
    var vbData: [VertexElement]
    var ibData: [QuadIndexList]
    var vb: MTLBuffer?
    var ib: MTLBuffer?
    var vbPlaneData: [VertexElement]
    var ibPlaneData: [QuadIndexList]
    ...
    init(device: MTLDevice, alert: (String) -> Void) {
        ...
        self.vbPlaneData = vbPlaneData
        let ibPlaneData: [QuadIndexList] = [
            QuadIndexList(0, 1, 2, 2, 1, 3)
        ]
        self.ibPlaneData = ibPlaneData
        ...
        self.stagedRes = [[MTLResource]](repeating: [], count: 2)
    }
    func available() -> Bool {
        self.pso != nil
    }
    func createSphere(device: MTLDevice) {
    ...
    func releaseSphere(device: MTLDevice) {
        if (ManualRetaining) {
            self.stagedRes[0].append(self.vb!)
            self.stagedRes[0].append(self.ib!)
        }
        self.vb = nil
        self.ib = nil
    }
    func createPlane(device: MTLDevice) {
    ...
    func releasePlane(device: MTLDevice) {
        if (ManualRetaining) {
            self.stagedRes[0].append(self.vbPlane!)
            self.stagedRes[0].append(self.ibPlane!)
        }
        self.vbPlane = nil
        self.ibPlane = nil
    }
...
// Some GPU workloads
let SPHERE_STACKS: Int = 200
let SPHERE_SLICES: Int = 240
...
class Metal: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        self.resource.stagedRes[1].removeAll(keepingCapacity: true)
        // Swap staging. Double buffering needs two frame interval.
        self.resource.stagedRes.swapAt(0, 1)
        
        // Create VB/IB at regular intervals
        if self.frameCount % 10 == 1 {
            self.resource.createSphere(device: self.device)
        }
        else if self.frameCount % 10 == 2 {
            self.resource.createPlane(device: self.device)
        }
        ...
        // Unretained CommandBuffer
        let cmdBuf = self.cmdQueue.makeCommandBufferWithUnretainedReferences()!
        ...
        if (self.resource.vb != nil && self.resource.ib != nil) {
        ...
        if (self.resource.vbPlane != nil && self.resource.ibPlane != nil) {
        ...
        // Release in-flight VB/IB
        if self.frameCount % 10 == 3 {
            self.resource.releaseSphere(device: self.device)
        }
        else if self.frameCount % 10 == 4 {
            self.resource.releasePlane(device: self.device)
        }
