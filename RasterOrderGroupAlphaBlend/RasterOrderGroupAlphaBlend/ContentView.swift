...
class MyResource {
    ...
    init(device: MTLDevice, alert: (String) -> Void) {
        ...
        instanceMat = instanceMat.map {_ in
            ...
            let sx = Float(0.30)//Float.random(in: 0.20...0.30, using: &rng)
            let sy = Float(0.30)//Float.random(in: 0.20...0.30, using: &rng)
            let sz = Float(0.30)//Float.random(in: 0.20...0.30, using: &rng)
...
class Metal: NSObject, MTKViewDelegate {
    ...
    init(_ parent: ContentView2) {
        ...
        if !device.areRasterOrderGroupsSupported {
            parent.enqueueAlert("Raster order groups not supported")
        }
